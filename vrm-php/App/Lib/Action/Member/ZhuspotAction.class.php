...
class ZhuspotAction extends MemberAction {
    ...
    function index() {
        ...
        foreach ($viewlist as $k => $view) {
            ...
            $vspotrow = M("Pano_zhu")->where($lwhere)->select();
    ...
    function view() {
        ...
        $roamrow = M("Pano_zhu")->where($roamwhere)->order("id")->select();
        ...
        foreach ($roamrow as $key => $roamvalue) {
			...
			$showtext = substring(SpHtml2Text($roamvalue['content']),0,45);
			$roamrow[$key]["showtext"] = $showtext;
        }
    ...
    function roam_edit() {
        ...
        if (I("post.dopost") == "save") {
            ...
            if ($spot_type == 1) {
                ...
                if (substr_count($spoturl, "station") > 0) {
                    if (is_file(APP_ROOT . $spoturl)) {
                        ...
                        $roamrow = M("Pano_zhu ")->where($roamwhere)->find();
            ...
            M("Pano_zhu")->where($where)->save($data);
        ...
        $roamrow = M("Pano_zhu")->where($roamwhere)->find();
        if(empty($roamrow['content'])) $roamrow['content'] = $roamrow['text'];
		$this->assign("roamrow", $roamrow);
    ...
    function roam_del() {
        ...
        M("Pano_zhu")->where($roamwhere)->delete();
