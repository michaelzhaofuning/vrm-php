 ...
 * @return string
 */
function UOne($url='', $vars='', $suffix=FALSE, $redirect=false, $domain=false) {
    // 解析URL
    $info = parse_url($url);
    $url = !empty($info['path']) ? $info['path'] : ACTION_NAME;
    if (isset($info['fragment'])) { // 解析锚点
...
/**
 * 对U函数产生的链接进行简化处理
 * @param string $url 调入的链接
 * @return string
 */
function kill_index_str_back($url) {
    if ($url == "/index") {
        return "/";
        exit();
    }
    $num = stripos($url, "/index", 1);
    if ($num) {
        $realurlarr = explode("?", $url);
        $realurl = $realurlarr[0];
        $long = strlen($realurl);
        if ($realurlarr[1] != "") {
            $enddata = "?" . $realurlarr[1];
        } else {
            $enddata = "";
        }
        if ($long > 6) {
            $endstr = substr($realurl, -6, 6);
            if ($endstr == "/index" || $endstr == "/Index") {
                $cutlen = $long - 6;
                $newurl = substr($realurl, 0, $cutlen) . $enddata;
                return kill_index_str_back($newurl);
            } else {
                return $url;
            }
        } else if ($realurl == "/index" || $realurl == "/Index") {
            $newurl = "/" . $enddata;
            return kill_index_str_back($newurl);
        } else {
            return $url;
